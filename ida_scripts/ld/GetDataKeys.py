...
from idc_bc695 import GetFunctionName, Byte
...
def dumpkvp(functionName, addr, key):
    ...
    if 'getMissionPlateFileName' in functionName:
        return
    if functionName.count('_') == 1 and key in functionName and functionName[-2:] == 'Ev':
        for (startea, endea) in Chunks(addr):
            for head in Heads(startea, endea):
                operand = GetDisasm(head)

                if 'LDR' in operand and 'R0, =(' in operand:
...
#     json.dump(files, fp, ensure_ascii=False, indent=2, encoding='utf-8')
